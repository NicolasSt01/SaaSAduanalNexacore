                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <div class="d-flex gap-1 justify-content-center flex-wrap">
                                                            ...
                                                            @endif 
                                                            {{-- 🔥 NUEVO: Indicador de expediente faltante --}}
                                                            {{--@if(is_null($incompleto->expediente) || empty($incompleto->expediente->numero_pedimento))
                                                                <span class="badge bg-warning text-dark" title="Falta Expediente/Pedimento">
                                            ...
                                            <div>
                                                ...
                                                <div class="thermo-meta mt-1">
                                                    ...
                                                    <span class="me-2"><i class="fas fa-tag"></i>
                                                        ...
                                                        {{-- 🔥 Indicador de datos faltantes con tooltip --}}
    @php
        $faltantes = [];
        foreach($registros as $reg) {
            if(empty($reg->bodega_id)) $faltantes[] = 'Bodega';
            if(is_null($reg->expediente) || empty($reg->expediente->numero_pedimento)) $faltantes[] = 'Asignar Pedimento';
        }
        $faltantes = array_unique($faltantes); // Eliminar duplicados
    @endphp
            ...
            <div class="modal-dialog modal-xl modal-dialog-centered">
                <div class="modal-content rounded-4 shadow-lg">
                    <div class="modal-header border-0 rounded-top-4 py-4 px-4"
                        ...
                        <h5 class="modal-title fw-bold">Detalles del Economico:
                            **{{ $first->num_thermo . "|" . $first->codigo_alpha }}**</h5>
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="mb-3" id="conceptosLista{{ $numThermo }}">
                                        @php
                                            ...
                                            // Obtener TODOS los conceptos de camión para TODAS las operaciones de este thermo
                                            ...
                                            $conceptosCamion = $conceptosCamion->unique('id'); // Evitar duplicados
                                        @endphp
        ...
        <div class="modal fade" id="conceptosModal{{ $modalId }}" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content rounded-4 shadow-lg">
                    <div class="modal-header bg-success text-white rounded-top-4">
                        <h5 class="modal-title">
                            <i class="fas fa-dollar-sign me-2"></i>Conceptos Adicionales - Economico {{ $thermo }} |
                            {{ $alpha }}
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body p-4">
                        <form action="{{ route('conceptos.store') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <input type="hidden" name="ambito" value="camion">
                            {{-- NUEVO: Selector de operación/factura --}}
                            <div class="mb-3">
                                <label class="form-label fw-bold">
                                ...
                                <select class="form-select" name="operacion_id" required>
                                    ...
                                    @foreach($registros as $exp)
                                        <option value="{{ $exp->id }}">
                                            {{ $exp->cliente->nombre_empresa ?? 'Sin cliente' }} -
                                            Factura #{{ $exp->num_factura }} -
                                            REF#{{ $exp->referencia }}
                                        </option>
                                    @endforeach
                                </select>
                                <small class="text-muted">Selecciona a qué factura se aplicará este concepto</small>
                            </div>
