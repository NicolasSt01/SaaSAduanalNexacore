        ...
        <nav class="flex mb-2" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        <i class="fas fa-chevron-right text-gray-400 text-xs mx-2"></i>
                        <span class="text-sm font-medium text-gray-700">Nueva Bodega</span>
                    </div>
                </li>
            </ol>
        </nav>
        <h1 class="text-3xl font-black text-gray-800 tracking-tight">Nueva <span class="text-indigo-600">Bodega</span></h1>
        <p class="text-sm text-gray-500 mt-2 font-medium">Registra una nueva bodega en el sistema.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-100">
            <h3 class="text-lg leading-6 font-bold text-gray-800 flex items-center gap-2">
                <i class="fas fa-warehouse text-indigo-500"></i> Formulario de Registro
            </h3>
        </div>

        <div class="p-6 sm:p-8">
            <form action="{{ route('bodegas.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="nombre_bodega" name="nombre_bodega" required value="{{ old('nombre_bodega') }}" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 p-3 border shadow-sm bg-gray-50/50" placeholder="Ej: Bodega Central">
                        @error('nombre_bodega')
                    ...
                    <div>
                        ...
                        <input type="text" id="tax_id" name="tax_id" value="{{ old('tax_id') }}" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 p-3 border shadow-sm bg-gray-50/50" placeholder="Equivalente al RFC en México">
                        @error('tax_id')
                            <p class="mt-1 text-xs text-rose-600 font-bold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="contacto" name="contacto" value="{{ old('contacto') }}" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 p-3 border shadow-sm bg-gray-50/50" placeholder="Ej: +52 555 123 4567">
                        @error('contacto')
                            <p class="mt-1 text-xs text-rose-600 font-bold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="domicilio" name="domicilio" rows="3" class="w-full rounded-xl border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 p-3 border shadow-sm bg-gray-50/50" placeholder="Dirección completa de la bodega">{{ old('domicilio') }}</textarea>
                        @error('domicilio')
                            <p class="mt-1 text-xs text-rose-600 font-bold">{{ $message }}</p>
                        @enderror
                    </div>
