            ...
            <div class="card border-0 shadow-sm bg-info text-white">
                <div class="card-body text-center">
                    ...
                    <h2 class="fw-bold mb-0">
                        @php
                            $cantidadConceptos = 0;
                            if(isset($detalleAdicionales) && is_array($detalleAdicionales)) {
                                foreach($detalleAdicionales as $concepto) {
                        ...
                        @forelse($operaciones as $index => $op)
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    @php
                                        $colorMod = match($op->modulacion) {
                                            'DESADUANAMIENTO LIBRE' => 'success',
                                            'RECONOCIMIENTO ADUANERO CONCLUIDO' => 'danger',
                                            'RECONOCIMIENTO ADUANERO' => 'warning text-dark',
                                            default => 'secondary'
                                        };
                                        $textoMod = match($op->modulacion) {
                                            'DESADUANAMIENTO LIBRE' => 'Verde',
                                            'RECONOCIMIENTO ADUANERO CONCLUIDO' => 'Rojo',
                                            'RECONOCIMIENTO ADUANERO' => 'En Proceso',
                                            default => 'Sin Módulo'
                                        };
                                        $iconMod = match($op->modulacion) {
                                            'DESADUANAMIENTO LIBRE' => 'check-circle',
                                            'RECONOCIMIENTO ADUANERO CONCLUIDO' => 'exclamation-triangle',
                                            'RECONOCIMIENTO ADUANERO' => 'clock',
                                            default => 'question'
                                        };
                                    @endphp
                                    <span class="badge bg-{{ $colorMod }}" 
                                          style="cursor: pointer;"
                                          onclick="loadModulacionModal('{{ $op->id }}')">
                                        <i class="fas fa-{{ $iconMod }} me-1"></i>{{ $textoMod }}
                                    </span>
                                </td>
                                {{--<td>
...
<div class="modal fade" id="modalModulacion" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg overflow-hidden" id="modalModulacionContent" style="border-radius: 15px;">
...
<script>
function loadModulacionModal(id) {
    const modalElement = document.getElementById('modalModulacion');
    ...
    document.getElementById('modalModulacionContent').innerHTML = `
        <div class="text-center py-5">
            ...
            <p class="mt-2 text-muted">Cargando detalles de modulación...</p>
        </div>`;

    modal.show();

    // Construir URL usando el ID directamente
    const url = `/finanzas/modal-modulacion/${id}`;
            ...
            if (data.success) {
                ...
                document.getElementById('modalModulacionContent').innerHTML = `
                    <div class="modal-body text-center py-4">
                        ...
                        <h5>Sin Datos</h5>
                        <p class="text-muted">${data.message || 'No se encontraron registros para esta operación.'}</p>
        ...
        .catch(error => {
            ...
            document.getElementById('modalModulacionContent').innerHTML = `
                <div class="modal-body text-center py-4">
                    ...
                    <h5>Error</h5>
                    <p class="text-muted">Hubo un problema al cargar los detalles.</p>
                    <button type="button" class="btn btn-secondary mt-3" data-bs-dismiss="modal">Cerrar</button>
                </div>`;
        });
}
