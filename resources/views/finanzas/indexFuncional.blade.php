...
<div class="container-fluid py-4">
        ...
        <div class="col-md-2">
            <div class="card border-0 shadow-sm" style="background-color: #6c757d;">
                <div class="card-body text-center text-white">
                    ...
                    <h2 class="mb-0 fw-bold">{{ $resumen->sum('adicionales') ?? 0 }}</h2>
                </div>
            </div>
        </div>
    </div>
    ...
    @else
        @php
            ...
            // Separar clientes completados y pendientes
            $clientesPendientes = [];
            $clientesCompletados = [];
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex gap-2">
                            <span class="badge bg-light text-dark badge-stat">
                                {{ $patentes->count() }} {{ $patentes->count() == 1 ? 'Patente' : 'Patentes' }}
                            </span>
                            <span class="badge bg-light text-dark badge-stat">
                                {{ $patentes->sum('expedientes_count') }} Expedientes
                            </span>
                            <span class="badge bg-light text-dark badge-stat">
                                {{ $patentes->sum('total_tramites') }} Trámites
                            </span>
                            <span class="badge bg-light text-dark badge-stat">
                                {{ $patentes->sum('rojos') }} Rojos
                            </span>
                            <span class="badge bg-light text-dark badge-stat">
                                {{ $patentes->sum('sobrepesos') ?? 0 }} Sobrepesos
                            </span>
                            <span class="badge bg-light text-dark badge-stat">
                                {{ $patentes->sum('adicionales') ?? 0 }} Adicionales
                            </span>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                @foreach($patentes as $patente)
                                    ...
                                    <tr class="patente-row {{ $rowClass }}">
                                        <td>
                                            <strong class="text-dark">{{ $patente['patente_numero'] }}</strong>
                                        </td>
                                        <td class="text-center">
                                            <span class="badge bg-secondary">{{ $expedientesTotal }}</span>
                                        </td>
                                        <td class="text-center">
                                            <span class="badge" style="background-color: #28a745;">{{ $patente['total_tramites'] }}</span>
                                        </td>
                                        <td class="text-center">
                                            <span class="badge" style="background-color: #dc3545;">{{ $patente['rojos'] }}</span>
                                        </td>
                                        <td class="text-center">
                                            <span class="badge" style="background-color: #ffc107; color: #000;">{{ $patente['sobrepesos'] ?? 0 }}</span>
                                        </td>
                                        <td class="text-center">
                                            <span class="badge" style="background-color: #6c757d;">{{ $patente['adicionales'] ?? 0 }}</span>
                                        </td>
                                        <td>
                                            <small class="text-muted">
                                                {{ $patente['fecha_apertura'] ? \Carbon\Carbon::parse($patente['fecha_apertura'])->format('d/m/Y') : 'N/A' }}
                                            </small>
                                        </td>
                                        <td>
                                            <small class="text-muted">
                                                {{ $patente['fecha_cierre'] ? \Carbon\Carbon::parse($patente['fecha_cierre'])->format('d/m/Y') : 'N/A' }}
                                            </small>
                                        </td>
                                        <td class="text-center">
                                            @if($patenteCompleta)
                                            ...
                                            @else
                                                <small class="text-muted">{{ $expedientesFacturados }}/{{ $expedientesTotal }}</small>
                                            @endif
                                        </td>
                                        <td class="text-center">
                            ...
                            <tfoot class="table-light">
                                <tr>
                                    ...
                                    <td class="text-center"><strong>{{ $patentes->sum('adicionales') ?? 0 }}</strong></td>
                                    <td colspan="4"></td>
                                </tr>
                            </tfoot>
                        </table>
