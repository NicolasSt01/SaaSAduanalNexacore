        ...
        <div class="col-xl-8 col-lg-7 mb-4">
            <div class="card shadow-sm h-100">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-primary">{{ $exportaciones->count() }} trámites</span>
                </div>
                <div class="card-body">
                    <div class="row">
                            ...
                            <div class="card h-100 border-start {{ $borderColor }} border-3">
                                <div class="card-body">
                                    <div class="d-flex align-items-center mb-2">
                                        <div class="flex-grow-1">
                                            <h6 class="card-title mb-0">{{ $exportacion->cliente->nombre_empresa }}</h6>
                                            <small class="text-muted">Estado: {{ ucfirst($exportacion->estado) }}</small>
                                        </div>
                                        <span class="badge {{ $badgeColor }}">{{ ucfirst($exportacion->prioridad) }}</span>
                                    </div>
                                    
                                    <p class="card-text small mb-1">
                                        <strong>Producto:</strong> {{ $exportacion->nombre_producto }}
                                    </p>
                                    <p class="card-text small mb-1">
                                        <strong>Factura:</strong> {{ $exportacion->num_factura }}
                                    </p>
                                    @if($exportacion->num_thermo)
                                    <p class="card-text small mb-2">
                                        <strong>Thermo:</strong> {{ $exportacion->num_thermo }}
                                    </p>
                                    @endif

                                    <div class="d-flex justify-content-between align-items-center">
                                        <span class="badge bg-light text-dark">#{{ $exportacion->id }}</span>
                                        
                                        @if($exportacion->estado == 'completado')
                                            <span class="badge bg-success">Finalizado</span>
                                        @else
                                            <a href="{{ route('documentador.trabajar', $exportacion->id) }}" 
                        ...
                        <div class="col-12">
                            <div class="text-center py-5">
                                ...
                                <p class="text-muted">No tienes exportaciones asignadas para hoy</p>
                            </div>
                        </div>
                        @endforelse
                    </div>
