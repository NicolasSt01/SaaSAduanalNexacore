    ...
    <div class="container-fluid py-3">
        ...
        <div class="row">
            ...
            <div class="col-lg-8 col-md-7">
                ...
                <div> <a href="{{ route('operaciones.create') }}" class="btn btn-primary btn-sm">
        ...
        <div class="col-xxl-3 col-lg-4 col-md-6 col-sm-6 mb-3 operacion-card"
            ...
            <div class="card h-100 shadow-sm 
                ...
                <div class="card-body p-3">
                    ...
                    <!-- Prioridad o Modulacion -->
                    @if (($operacion->estado ?? '')=='terminado')
                        <div class="mb-2">
                            <span class="badge bg-dark modulacion-badge"
                                  id="modulacion-{{ $operacion->id }}">
                                Modulacion: {{ $operacion->modulacion ?? 'Pendiente' }}
                            </span>
                        </div>
                    @else
                        <div class="mb-2">
...
@section('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const progresoChart = new Chart(ctxProgreso, {
                type: 'doughnut',
                data: {
            ...
            const aduanaChart = new Chart(ctxAduana, {
                type: 'doughnut',
                data: {
                    labels: ['Verdes', 'Rojos'],
                    datasets: [{
    ...
    <script>
    document.addEventListener('DOMContentLoaded', function () {
        // Actualizar modulaciones cada 5 segundos
        setInterval(() => {
            fetch("{{ route('operaciones.modulaciones') }}")
                .then(response => response.json())
                .then(data => {
                    data.forEach(exp => {
                        const badge = document.getElementById(`modulacion-${exp.id}`);
                        if (badge) {
                            let colorClass = 'bg-dark';
                            if (exp.modulacion === 'Verde') colorClass = 'bg-success';
                            else if (exp.modulacion === 'Rojo') colorClass = 'bg-danger';
                            else if (exp.estado === 'terminado') colorClass = 'bg-secondary';

                            badge.className = `badge ${colorClass}`;
                            badge.textContent = `Modulación: ${exp.modulacion ?? 'Pendiente'}`;
                        }
                    });
                })
                .catch(err => console.error("Error actualizando modulaciones:", err));
        }, 5000);
    });
</script>
