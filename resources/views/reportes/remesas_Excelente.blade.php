...
function initializeChart() {
    ...
    const goalLine   = Array(data.length).fill(GOAL);
    const buenoLine  = Array(data.length).fill(BUENO);
    const maloLine   = Array(data.length).fill(MALO);
            ...
            datasets: [
    {
        ...
        backgroundColor: data.map(value => {
            if (value >= GOAL) return 'rgba(25, 135, 84, 0.85)';   // verde
            if (value >= BUENO) return 'rgba(255, 193, 7, 0.85)'; // amarillo
            return 'rgba(220, 53, 69, 0.85)';                     // rojo
        }),
        borderColor: data.map(value => {
            if (value >= GOAL) return 'rgba(25, 135, 84, 1)';
            if (value >= BUENO) return 'rgba(255, 193, 7, 1)';
    ...
    callbacks: {
        label: function(context) {
            ...
            if (value >= GOAL) return `Remesas: ${value} ✔ Meta alcanzada`;
            if (value >= BUENO) return `Remesas: ${value} ⚠ Nivel aceptable`;
            return `Remesas: ${value} ❌ Nivel crítico`;
        }
    }
}
            },
