...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;background:#fff;">
    <tr><td style="background:linear-gradient(135deg,#4f46e5,#7c3aed);padding:40px 30px;text-align:center;">
        ...
        <h1 style="color:#fff;margin:0;font-size:22px;font-weight:800;">Instrucciones de Pago</h1>
        <p style="color:rgba(255,255,255,0.8);margin:8px 0 0;font-size:14px;">Plan: {{ $suscripcion->plan->nombre }}</p>
    </td></tr>
    <tr><td style="padding:30px;">
        <p style="color:#374151;font-size:15px;line-height:1.6;">Hola <strong>{{ $suscripcion->tenant->nombre_empresa }}</strong>,</p>
        <p style="color:#374151;font-size:15px;line-height:1.6;">Se ha generado una nueva suscripción para tu cuenta. A continuación encontrarás los detalles para realizar el pago:</p>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
            <tr><td colspan="2" style="background:#f9fafb;padding:12px 16px;font-weight:800;color:#4f46e5;font-size:12px;text-transform:uppercase;letter-spacing:1px;">Detalle del Pago</td></tr>
            <tr><td style="padding:10px 16px;border-top:1px solid #e5e7eb;color:#6b7280;font-size:13px;width:40%;">Subtotal</td><td style="padding:10px 16px;border-top:1px solid #e5e7eb;color:#111827;font-weight:700;font-size:14px;">${{ number_format($suscripcion->monto_base, 2) }} MXN</td></tr>
            <tr><td style="padding:10px 16px;border-top:1px solid #e5e7eb;color:#6b7280;font-size:13px;">IVA ({{ $config->iva_porcentaje }}%)</td><td style="padding:10px 16px;border-top:1px solid #e5e7eb;color:#111827;font-weight:700;font-size:14px;">${{ number_format($suscripcion->monto_iva, 2) }} MXN</td></tr>
            <tr style="background:#eef2ff;"><td style="padding:12px 16px;border-top:2px solid #4f46e5;color:#4f46e5;font-weight:800;font-size:14px;">TOTAL</td><td style="padding:12px 16px;border-top:2px solid #4f46e5;color:#4f46e5;font-weight:800;font-size:18px;">${{ number_format($suscripcion->monto_total, 2) }} MXN</td></tr>
        </table>
        ...
        <div style="background:#fef3c7;border:1px solid #fbbf24;border-radius:12px;padding:16px;margin:24px 0;text-align:center;">
            ...
            <p style="margin:8px 0 0;color:#92400e;font-size:24px;font-weight:900;font-family:monospace;letter-spacing:2px;">{{ $suscripcion->referencia_pago }}</p>
        ...
        <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:12px;padding:16px;margin:24px 0;">
            <p style="margin:0;color:#166534;font-size:13px;font-weight:700;"><i>✓</i> Pasos para pagar:</p>
            <ol style="margin:8px 0 0;padding-left:20px;color:#166534;font-size:13px;line-height:1.8;">
                <li>Realiza una transferencia bancaria por el monto total</li>
                <li>En el campo <strong>concepto/referencia</strong> coloca: <strong>{{ $suscripcion->referencia_pago }}</strong></li>
                <li>Tu suscripción se activará una vez confirmado el pago</li>
            </ol>
        </div>
    </td></tr>
    <tr><td style="background:#f9fafb;padding:20px 30px;text-align:center;border-top:1px solid #e5e7eb;">
        ...
        <p style="margin:4px 0 0;color:#d1d5db;font-size:10px;">{{ $config->notas_legales ?? '' }}</p>
    </td></tr>
</table>
</body></html>
